...
template<int N, int numblock, typename dataType1>
void collideWithForceSIMD(lbmRD3Q41<dataType1> &lbModel,
                          ...
                          dataType1 F3,
                          dataType1 dt) {
  alignas(32) dataType1 rho[VECT_LENGTH];
  alignas(32) dataType1 uX[VECT_LENGTH];
  alignas(32) dataType1 uY[VECT_LENGTH];
  alignas(32) dataType1 uZ[VECT_LENGTH];
  alignas(32) dataType1 theta[VECT_LENGTH];
  ...
  for (int i3 = myGrid.nB3; i3 <= myGrid.nE3; i3++) {
    for (int i2 = myGrid.nB2; i2 <= myGrid.nE2; i2++) {
      for (int i1 = myGrid.nB1; i1 <= myGrid.nE1; i1 = i1 + 4) {
        getHydroMomentsFromNodeWithForce(lbModel, myGrid,
                                         VECT_LENGTH,
                                         i1, i2, i3, rho,
                                         uX, uY, uZ, theta,
                                         F1, F2, F3, dt);
  ...
  //          theta[index] = lbModel.theta0;
        ...
        for (int index = 0; index < VECT_LENGTH; index++) {
          myGrid(i1 + index, i2, i3, lbModel.G0, myGrid.node[lbModel.G0],
                 lbModel.CENTER_DV_ZERO_ZERO_ZERO)
              = oneMinustwoBeta *
              myGrid(i1 + index, i2, i3, lbModel.G0, myGrid.node[lbModel.G0],
                     lbModel.CENTER_DV_ZERO_ZERO_ZERO)
              + twoBeta
          ...
          // oneMinustwoBeta*SC1+twoBeta*fEq
          _temp8 = ADD_PD(_temp5, _temp6);
          // oneMinustwoBeta*SC1+twoBeta*fEq +twoBetaTau*fTempF
  ...
  for (int i3 = myGrid.nB3; i3 <= myGrid.nE3; i3++) {
    for (int i2 = myGrid.nB2; i2 <= myGrid.nE2; i2++) {
      for (int i1 = myGrid.nB1; i1 <= myGrid.nE1; i1 = i1 + 4) {
        getHydroMomentsFromCellWithForce(lbModel, myGrid,
                                         VECT_LENGTH,
                                         i1, i2, i3, rho,
                                         uX, uY, uZ, theta,
                                         F1, F2, F3, dt);
...
template <int N,int numblock, typename dataType1>
void copyFromNodeTo4Pt41Array(lbmRD3Q41<dataType1> &lbModel, gridBCC3D<N, numblock, dataType1> &myGrid,int VECT_LENGTH,int i1, int i2, int i3, dataType1 feq[][41])
{
    for(int i=0;i<VECT_LENGTH;i++)
         feq[i][lbModel.DV_ZERO_ZERO_ZERO] = myGrid(i1+i,i2,i3,lbModel.G0,myGrid.node[lbModel.G0],lbModel.CENTER_DV_ZERO_ZERO_ZERO);

    for(int i=0;i<VECT_LENGTH;i++)
    {
        unsigned long long int index = myGrid.getIndex(i1+i,i2,i3,lbModel.G1,myGrid.node[lbModel.G1],  0);
        for(int dv=0;dv<4;dv++)
         feq[i][1+dv]  =    myGrid.value(index+dv) ;
    }
    ...
    for(int i=0;i<VECT_LENGTH;i++)
    {
        ...
        for(int dv=0;dv<4;dv++)
         feq[i][5+dv]  =     myGrid.value(index+dv) ;
    }
    ...
    for(int i=0;i<VECT_LENGTH;i++)
    {
        ...
        for(int dv=0;dv<4;dv++)
         feq[i][9+dv]  =        myGrid.value(index+dv) ;
    }
    ...
    for(int i=0;i<VECT_LENGTH;i++)
    {
        ...
        for(int dv=0;dv<4;dv++)
         feq[i][13+dv]  =      myGrid.value(index+dv) ;
    }
    ...
    for(int i=0;i<VECT_LENGTH;i++)
    {
        ...
        for(int dv=0;dv<4;dv++)
         feq[i][17+dv]  =       myGrid.value(index+dv) ;
    }
    ...
    for(int i=0;i<VECT_LENGTH;i++)
    {
        ...
        for(int dv=0;dv<4;dv++)
         feq[i][21+dv]  =       myGrid.value(index+dv) ;
    }
    ...
    for(int i=0;i<VECT_LENGTH;i++)
    {
        ...
        for(int dv=0;dv<4;dv++)
         feq[i][33+dv]  =       myGrid.value(index+dv) ;
    }
    for(int i=0;i<VECT_LENGTH;i++)
    {
        unsigned long long int index = myGrid.getIndex(i1+i,i2,i3,lbModel.G10,myGrid.node[lbModel.G10],  0);
        for(int dv=0;dv<4;dv++)
         feq[i][37+dv]  =      myGrid.value(index+dv) ;
    }
}

template <int N,int numblock, typename dataType1>
void copyFromCellTo4Pt41Array(lbmRD3Q41<dataType1> &lbModel, gridBCC3D<N, numblock, dataType1> &myGrid,int VECT_LENGTH,int i1, int i2, int i3, dataType1 feq[][41])
{
    for(int i=0;i<VECT_LENGTH;i++)
         feq[i][lbModel.DV_ZERO_ZERO_ZERO] = myGrid(i1+i,i2,i3,lbModel.G0,myGrid.cell[lbModel.G0],lbModel.CENTER_DV_ZERO_ZERO_ZERO);
...
// template <int N,int numblock, typename dataType1>
// void collideWithForceEntropicSIMD(lbmRD3Q41<dataType1> &lbModel, gridBCC3D<N, numblock, dataType1> &myGrid,int VECT_LENGTH, dataType1 beta, dataType1 betaTau , dataType1 F1, dataType1 F2, dataType1 F3)
// {
//     dataType1 fTempF[4][lbModel.dvN] ;
//     dataType1 f_eq[4][lbModel.dvN];
//     dataType1 f_i[4][lbModel.dvN];
...
//                     theta[index] = lbModel.theta0;
//
//                 copyFromNodeTo4Pt41Array(lbModel,myGrid,VECT_LENGTH,i1,i2,i3,f_i);
//
// //                 getFEqSIMD4Pt41Array(lbModel,VECT_LENGTH,rho,uX,uY,uZ,theta,f_eq);
//
//                 getFEqSIMD(lbModel,VECT_LENGTH,rho,uX,uY,uZ,theta);
//                 copyFrom4To41Array(lbModel,f_eq);
//
//                 getfTempFSIMD(lbModel,rho,F1,F2,F3,fTempF);
//
//
//                for(int index=0;index<VECT_LENGTH;index++)
//                {
//                 for(int dv=0;dv<lbModel.dvN;dv++)
//                  x_i[dv] =(f_eq[index][dv]/f_i[index][dv] - 1.0);
//
//                 for(int dv=0;dv<lbModel.dvN;dv++)
//                 {
//                  alpha[index] = 2.0;
//                  if( fabs(x_i[dv]) > 0.01)
//                  {
//                      calculateAlpha(lbModel,x_i,f_i,alpha[index],index);
//                      break;
//                  }
//                 }
//                }
...
// //                  if( fabs(x_i[dv]) > 0.01)
// //                  {
// //                      calculateAlpha(lbModel,x_i,f_i,alpha[0],0);
// //                      break;
// //                  }
// //                 }
// //
// //                 for(int dv=0;dv<lbModel.dvN;dv++)
// //                  x_i[dv] =(f_eq[1][dv]/f_i[1][dv] - 1.0);
// //
// //                 for(int dv=0;dv<lbModel.dvN;dv++)
// //                 {
// //                  alpha[1] = 2.0;
// //                  if( fabs(x_i[dv]) > 0.01)
// //                  {
// //                      calculateAlpha(lbModel,x_i,f_i,alpha[1],1);
// //                      break;
// //                  }
// //                 }
// //
// //                 for(int dv=0;dv<lbModel.dvN;dv++)
// //                  x_i[dv] =(f_eq[2][dv]/f_i[2][dv] - 1.0);
// //
// //                 for(int dv=0;dv<lbModel.dvN;dv++)
// //                 {
// //                  alpha[2] = 2.0;
// //                  if( fabs(x_i[dv]) > 0.01)
// //                  {
// //                      calculateAlpha(lbModel,x_i,f_i,alpha[2],2);
// //                      break;
// //                  }
// //                 }
// //
// //                 for(int dv=0;dv<lbModel.dvN;dv++)
// //                  x_i[dv] =(f_eq[3][dv]/f_i[3][dv] - 1.0);
// //
// //                 for(int dv=0;dv<lbModel.dvN;dv++)
// //                 {
// //                  alpha[3] = 2.0;
// //                  if( fabs(x_i[dv]) > 0.01)
// //                  {
// //                      calculateAlpha(lbModel,x_i,f_i,alpha[3],3);
// //                      break;
// //                  }
// //                 }
//
//                 _temp8 = SET1_PD(beta);              // beta
//                 _temp6 = SET1_PD(1.0);               // 1
//                pointer1 = &alpha[0];
//                 _temp7 = LOAD_PD(pointer1);          // alpha    */
// //                 _temp7 = SET1_PD(2.0);
//                 _temp4 = SET1_PD(betaTau);           // betaTau
//
//                 _temp1 = MUL_PD(_temp7,_temp4);      // alphaBetaTau
//                 _temp2 = MUL_PD(_temp7,_temp8);      // alphaBeta
//                 _temp3 = SUB_PD(_temp6,_temp2);      // oneMinusalphaBeta
//
//                 ///////////
//                 // CENTER /
//                 ///////////
...
//                 pointer1 = &tempArray[0];
//                 for(int index=0;index<VECT_LENGTH;index++)
//                   tempArray[index] =  lbModel.fTemp0 [index][lbModel.CENTER_DV_ZERO_ZERO_ZERO];
//                 _temp5 = LOAD_PD(pointer1);          // fEq
//                 for(int index=0;index<VECT_LENGTH;index++)
//                   tempArray[index] =  myGrid(i1+index,i2,i3,lbModel.G0 ,myGrid.node[lbModel.G0 ],lbModel.CENTER_DV_ZERO_ZERO_ZERO);
//                 _temp6 = LOAD_PD(pointer1);          // G0
//
//
//                 _temp6 = MUL_PD(_temp3,_temp6);          // G0*1-alphaBeta
//                 _temp5 = MUL_PD(_temp2,_temp5);          // fEq*alphaBeta
//                 _temp5 = ADD_PD(_temp6,_temp5);          // G0
//                 STORE_PD(pointer1,_temp5);
//                 for(int index=0;index<VECT_LENGTH;index++)
//                  myGrid(i1+index,i2,i3,lbModel.G0 ,myGrid.node[lbModel.G0 ],lbModel.CENTER_DV_ZERO_ZERO_ZERO) =  tempArray[index];
//
//
//                 /////////
//                 // G1 //
...
//                     theta[index] = lbModel.theta0;
//
//                 copyFromCellTo4Pt41Array(lbModel,myGrid,VECT_LENGTH,i1,i2,i3,f_i);
//
//
//                 getFEqSIMD(lbModel,VECT_LENGTH,rho,uX,uY,uZ,theta);
//                 copyFrom4To41Array(lbModel,f_eq);
...
//                 _temp5 = LOAD_PD(pointer1);          // fEq
//                 for(int index=0;index<VECT_LENGTH;index++)
//                   tempArray[index] =  myGrid(i1+index,i2,i3,lbModel.G0 ,myGrid.cell[lbModel.G0 ],lbModel.CENTER_DV_ZERO_ZERO_ZERO);
//                 _temp6 = LOAD_PD(pointer1);          // G0
//
//
//                 _temp6 = MUL_PD(_temp3,_temp6);          // G0*1-alphaBeta
...
//                 STORE_PD(pointer1,_temp5);
//                  for(int index=0;index<VECT_LENGTH;index++)
//                  myGrid(i1+index,i2,i3,lbModel.G0 ,myGrid.cell[lbModel.G0 ],lbModel.CENTER_DV_ZERO_ZERO_ZERO) =  tempArray[index];
//
//                 /////////
//                 // G1 //
//                 /////////
...
void collideWithForceEntropicSIMD(lbmRD3Q41<dataType1> &lbModel,
  ...
  alignas(32) dataType1 alphaBetaTauArray[4];
...
//           theta[index] = lbModel.theta0;
        ...
        getFEqSIMD(lbModel, VECT_LENGTH, rho, uX, uY, uZ, theta, countFeq);
	countFeq++;
        ...
        for (int index = 0; index < VECT_LENGTH; index++) {
          ...
          alphaBetaTauArray[index]  = alpha[index]*betaTau;
        }

         pointer1 = &alphaBetaTauArray[0];
        _temp1 = LOAD_PD(pointer1); // alphaBetaTau
...
template void getfTempFSIMD<double>(lbmRD3Q41<double> &,
double *,
double,
double,
double);
